...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.window.registerWebviewViewProvider(HamsterViewProvider.viewType, provider)
  );
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeTextDocument((event) => {
      ...
      const added = event.contentChanges.reduce(
        (sum, c) => sum + Math.max(0, c.text.length - c.rangeLength),
        0
      );
      if (added > 0) {
...
class HamsterViewProvider implements vscode.WebviewViewProvider {
  ...
  updateSpeed(cps: number) {
    const idleThreshold = vscode.workspace
      .getConfiguration('hamsterRun')
      .get<number>('idleThreshold', 0.2);
    this.view?.webview.postMessage({ type: 'speed', cps, idleThreshold });
  }

  private getHtml(webview: vscode.Webview): string {
    const mediaUri = (p: string) =>
      webview.asWebviewUri(vscode.Uri.joinPath(this.context.extensionUri, 'media', p)).toString();
